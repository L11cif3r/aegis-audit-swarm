    ...
    <div className="flex flex-col gap-7">
      ...
      <Reveal delay={0.05}>
        <div className="grid grid-cols-2 gap-3">
          <Stat label="Blocked" value={totalBlocked} icon={ShieldAlert} tone="bad" sub="Total mitigated" />
          <Stat label="Injections" value={injections} icon={ScanLine} tone="warn" sub="Jailbreak patterns" />
          <Stat label="Secrets" value={secrets} icon={KeyRound} tone="brand" sub="Keys redacted" />
          <Stat label="PII Cleaned" value={pii} icon={Fingerprint} tone="ok" sub="Identifiers" />
        </div>
      </Reveal>
      ...
      <Reveal delay={0.1}>
        <Panel>
          ...
          <div className="flex flex-col divide-y divide-[var(--hairline)] mt-2">
            {policies.map((p) => (
              <div key={p.name} className="py-3 flex justify-between items-center">
                <div>
        ...
        <Panel>
          ...
          <div className="flex flex-col gap-2 mt-3">
            {blockedLogs.length === 0 ? (
              <EmptyState>No active threats detected.</EmptyState>
            ) : (
              blockedLogs.slice(0, 6).map((log, i) => (
                <motion.div
                  key={log.id || i}
